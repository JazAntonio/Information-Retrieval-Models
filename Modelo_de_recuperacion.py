...
def modelo_recuperacion(consulta, docs_maximos, documentos_relevantes):
    ...
    aciertos = 0
    for doc_rel in documentos_relevantes:
        if doc_rel in claves:
            aciertos += 1
    R = aciertos/len(documentos_relevantes)

    print(f"Para la {consulta} el modelo recuperó los siguientes documentos:")
    print("Documento  -  Similitud(distancia coseno)")
    for k in range(10):
        if claves[k] in documentos_relevantes:
            print(str(claves[k]).rjust(3), "\t\t:\t", "{:.5f}".format(valores[k]), "<--- Documento relevante")
        else:
            print(str(claves[k]).rjust(3), "\t\t:\t", "{:.5f}".format(valores[k]))

    print(f"Total de documentos relevantes obtenidos: {aciertos} de {len(documentos_relevantes)}")
    print(f"El recall (R) es de: {R}")
    P = aciertos/10
    print(f"La precisión (P) es de: {P}")
    try:
        print(f"El valor de F1 es de: {(2*P*R)/(P+R)}")
    except:
        print("El valor de F1 no es posible calcularlo debido a que en este caso R=P=0")
...
bolsa_conteo.to_csv('Bolsa_de_palabras.csv', index=True)

bolsa_tf_idf = bolsa_tf*bolsa_idf
bolsa_tf_idf.to_csv('Pesado_tf_idf.csv', index=True)

consultas_binario = consultas_binario(path, stopwords, cabecera)
consultas_binario.to_csv('Consultas_binario.csv', index=True)

for k in range(1,10):
    modelo_recuperacion(f"Consulta {k}", 10, documentos_relevantes[k-1])
    print("\n")